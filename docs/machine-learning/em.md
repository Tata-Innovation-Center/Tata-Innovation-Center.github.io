...
Consider for example a probabilistic language model of news articles $$p$$ assigns probabilities to sequences of words $$x_1,...,x_n$$. We can, among other things, sample from $$p$$ to generate various kinds of sentences.". Each article $$x$$ typically focuses on a specific topic $$t$$, e.g. finance, sports, politics. Using this prior knowledge, we may build a more accurate model $$p(x \mid t)p(t)$$, in which we have introduced an additional, unobserved variable $$t$$. This model can be more accurate, because we can now learn a separate $$p(x \mid t)$$ for each topic, rather than trying to model everything with one $$p(x)$$.

However, since $$t$$ is unobserved, we cannot directly use the learning methods that we have so far. In fact, the unobserved variables make learning much more difficult; in this post, we will look at how to use and how to learn models that involve latent variables.
...
There exist both discriminative and generative LVMs, although here we will focus on the latter (the key ideas hold for discriminative models as well).
...
In a GMM, each data point is a tuple $$(x_i, z_i)$$ with $$x_i \in \Re^d$$ and $$z_i \in \{1, 2, \dotsc, K\}$$ ($$z_i$$ is discrete). 
...
This optimization objective is considerably more difficult than regular log-likelihood, For one, we can see that the summation inside the log makes it impossible to decompose $$p(x)$$ into a sum of log-factors. 
...
Since the objective is non-convex, we will resort to approximate learning algorithms. These methods are widely used in practice and are quite effective.
...
Since the summation is now outside the log, we can maximize the expected log-likelihood.
